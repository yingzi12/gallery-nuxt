...
function updateStatus(album: any, statusChoise: number) {
  ...
  $q.dialog({
    ...
    const response = await axios.get(`/api/addAlbum/updateStatus?id=${album.id}&status=${statusChoise}`, {
      method: 'get',
      headers: {
        ...
        <q-item>
          ...
          <q-item-section>
            ...
            <q-item-label class="q-mt-xs text-body2 text-weight-bold text-primary text-uppercase">
              <span class="cursor-pointer"><a :href="'/addAlbumImage?aid='+album.id+'&title='+album.title">照片： {{
                  album.numberPhotos
                }}</a></span>
            </q-item-label>
          </q-item-section>
